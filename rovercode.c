...
//My code -------------------------------------------------
#define CMDSeismicActivity 59
#define CMDSpatialVortex 60
//---------------------------------------------------------
...
void InitAppConfig(void);  void InitializeBoard(void); void ProcessIO(void); void POSTTCPchar(char b); void POSTTCPhead(int len, int id);
void getZvalue(void); double getRotation(void);

//extern volatile BYTE GPSRX[8];
//extern volatile int GPSRXcount;
//extern volatile int GPSRXcount;
volatile int LINEA, LINEB; volatile int LINEAa, LINEBa; volatile int emf1v, emf2v;; volatile int photov, thermv; volatile int lightav, lightbv; volatile char ToggleLine = 1;
...
char Zmsb = 0; 
char Zlsb = 0;
int SeismicCounter = 0;
int SpatialCounter = 0;
int SeismicFlag = 0; //if 0, no seismic. if 1, get acc vals
int SpatialFlag = 0;
int Zvalue = 0;//stores next z value
int magX;
int magY;
volatile double sCompass;
volatile double storedCompass = 0;
volatile int Xmsb;
volatile int Xlsb;
volatile int Ymsb;
volatile int Ylsb;
...
// My code---------------------------------------------------------------------------
	  //Note to programmer: pic runs at 40KHz
    if(SeismicFlag == 1){
	    SeismicCounter++;
	    if(SeismicCounter >=800){//Sets sample speed
			getZvalue();
	        LED1_IO = 0;
			POSTTCPhead(2, CMDSeismicActivity);
			POSTTCPchar(Zmsb);
			POSTTCPchar(Zlsb);
	        SeismicCounter = 0;
    	}
    ...
    if (SpatialFlag == 1){
    	SpatialCounter++;
    	if(SpatialCounter >=800){
    		sCompass = getRotation();
    		LED1_IO = 1;
    		SpatialCounter = 0;
			POSTTCPhead(4, CMDSpatialVortex);
			POSTTCPchar(Xmsb);
			POSTTCPchar(Xlsb);
			POSTTCPchar(Ymsb);
			POSTTCPchar(Ylsb);
	...
	if (ToggleLine)
	{
		if (MotorPWMcount > 256) { if (setLineLed == 1) setLineLed = 2; }
		else { if (setLineLed == 0) setLineLed = 3; }
	}
	if (AD1CON2bits.BUFS)
	{
		if (ToggleLine)
...
void getZvalue(void){
    ...
    i = I2GET(0); I2P();
	if (i & 8) {
		I2S(); I2send(0x38); I2send(0x05); I2SR(); I2send(0x39);
		Zmsb = (char)I2GET(1);
		Zlsb = (char)I2GET(0);
		I2P();
	}
...
double getRotation(void){
	...
	I2S(); I2send(0x1c); I2send(0x00); I2SR(); I2send(0x1d);
	if ((I2GET(0) & 0x8) == 0){
			I2P();
			//return storedCompass;
		}
	else{
		I2P();
	...
	double compass = atan2(magYN, magXN) * (180/3.14);
	storedCompass = compass;
...
void processcommand(void) {		// the main routine which processes commands
	...
	{
//My Code -----------------------------------------------
	case CMDSeismicActivity:
		if (SeismicFlag == 0) SeismicFlag = 1;
		else SeismicFlag = 0;
		break;

	case CMDSpatialVortex:
		if (SpatialFlag == 0) SpatialFlag = 1;
		else SpatialFlag = 0;
		break;
//-------------------------------------------------------
